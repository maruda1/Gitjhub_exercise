...
- (NSAttributedString *)labelAttributedString:(NSString *)labelString //ラベルの文字列を属性付きで変換
{
    //ラベル文字列
    NSString *text = (labelString == nil) ? @"" :labelString; //参考演算子のサンプルとして、普通のif文でおけ
    //フォントを指定
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.httpErrorMessage){
        ...
        dispatch_async(self.imageQueue, ^{
            ...
            if([[tweetDictionary allKeys] containsObject:@"retweeted_stasus"]){
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    return tweetTextLabelHeight + 35; //セルのツイート本文ラベル以外の高さが合計で35ピクセル
}
